...
const WEB_APP_URL ='https://script.google.com/macros/s/AKfycbzq9SWm2mEBe_gsusJKNEj7hlORO29BejRrOI7CoapwBj145UCyUBccmzdv4pzLAHlW/exec';const IMAGE_API_URL = WEB_APP_URL + '?mode=images';const BOOK_API_URL = WEB_APP_URL + '?mode=books';const NEWS_API_URL = WEB_APP_URL + '?mode=news';

const toggle = document.querySelector('.menu-toggle');const nav = document.querySelector('.main-nav');

if (toggle && nav) {toggle.addEventListener('click', () => {const isOpen = nav.classList.toggle('open');toggle.setAttribute('aria-expanded', String(isOpen));toggle.textContent = isOpen ? '✕' : '☰';});
...
async function loadWebsiteImages() {try {const response = await fetch(IMAGE_API_URL + '&_t=' + Date.now(),{method: 'GET',cache: 'no-store'});
...
if (heroOverlayUrl) {const overlay = document.getElementById('websiteHeroOverlay');

if (overlay) {const heroImage = new Image();
...
} catch (error) {console.error('โหลด URL รูปภาพเว็บไซต์ไม่สำเร็จ:',error);}}

let newsSlides = [];let newsIndex = 0;let newsTimer = null;let newsAutoEnabled = false;let newsAutoStoppedByUser = false;let newsPopupOpen = false;

function stopNewsAutoSlide() {if (newsTimer) {clearInterval(newsTimer);newsTimer = null;}}

function startNewsAutoSlide() {stopNewsAutoSlide();
...
function stopNewsAutoByUser() {newsAutoStoppedByUser = true;stopNewsAutoSlide();}

function normalizeNewsItem(item, index) {if (typeof item === 'string') {return {newsNo: index + 1,title: ข่าวสาร ${index + 1},image: String(item).trim(),detail: '',detailUrl: '',date: ''};}
...
async function loadNews() {const slider = document.getElementById('newsSlider');const slidesBox = document.getElementById('newsSlides');if (!slider || !slidesBox) return;
...
function renderNews() {const slidesBox = document.getElementById('newsSlides');const dots = document.getElementById('newsDots');if (!slidesBox || !dots || !newsSlides.length) return;
...
async function openNewsPopup(item) {if (!item) return;
...
function bindNewsControls() {const slider = document.getElementById('newsSlider');const prev = document.getElementById('newsPrev');const next = document.getElementById('newsNext');
...
let books = [];let activeBookIndex = 0;let bookTimer = null;

const escapeHtml = value => String(value ?? '').replaceAll('&', '&').replaceAll('<', '<').replaceAll('>', '>').replaceAll('"', '"').replaceAll("'", ''');

function normalizeBook(row, index) {return {id: String(row.bookId || row.id || row.number || row['เล่มที่'] || index + 1),category: String(row.category || row['หมวด'] || '').trim(),title: String(row.title || row.bookName || row['ชื่อหนังสือ'] || '').trim(),image: String(row.image || row.imageUrl || row['URL รูปปก'] || '').trim(),detail: String(row.detail || row.description || row['รายละเอียดที่น่าสนใจ'] || '').trim()};}

async function loadBooks() {const slides = document.getElementById('bookSlides');const grid = document.getElementById('allBooksGrid');if (!slides || !grid) return;
...
function renderBookSlider() {const slides = document.getElementById('bookSlides');const dots = document.getElementById('bookDots');if (!slides || !dots || !books.length) return;

slides.innerHTML = books.map((book, index) => {
  const distance = circularDistance(index, activeBookIndex, books.length);

const positionClass =distance === 0 ? 'is-active' :distance === -1 ? 'is-prev' :distance === -2 ? 'is-far-prev' :distance === -3 ? 'is-super-far-prev' :distance === 1 ? 'is-next' :distance === 2 ? 'is-far-next' :distance === 3 ? 'is-super-far-next' :'is-hidden';
...
function circularDistance(index, active, length) {let distance = index - active;if (distance > length / 2) distance -= length;if (distance < -length / 2) distance += length;return distance;}

function setActiveBook(index) {if (!books.length) return;activeBookIndex = (index + books.length) % books.length;renderBookSlider();restartBookAutoSlide();}

function startBookAutoSlide() {clearInterval(bookTimer);if (books.length <= 1) return;bookTimer = setInterval(() => {activeBookIndex = (activeBookIndex + 1) % books.length;renderBookSlider();}, 3500);}

function restartBookAutoSlide() {startBookAutoSlide();}

function renderAllBooks() {const grid = document.getElementById('allBooksGrid');if (!grid) return;
...
function openBookDetail(index) {const book = books[index];const modal = document.getElementById('bookDetailModal');const content = document.getElementById('bookDetailContent');if (!book || !modal || !content) return;
...
function closeBookDetail() {const modal = document.getElementById('bookDetailModal');if (!modal) return;modal.classList.remove('open');modal.setAttribute('aria-hidden', 'true');document.body.classList.remove('modal-open');}

function bindBookControls() {document.getElementById('bookPrev')?.addEventListener('click', () => {setActiveBook(activeBookIndex - 1);});
...
if (toggleButton && panel) {toggleButton.addEventListener('click', () => {const willOpen = panel.hasAttribute('hidden');
...
document.getElementById('bookDetailModal')?.addEventListener('click', event => {if (event.target.id === 'bookDetailModal') {closeBookDetail();}});

document.addEventListener('keydown', event => {if (event.key === 'Escape') {closeBookDetail();}});}

document.addEventListener('DOMContentLoaded', () => {bindNewsControls();bindBookControls();loadWebsiteImages();loadNews();loadBooks();});})();
